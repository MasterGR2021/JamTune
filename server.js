...
app.post("/search", (req, res) => {
  const optionsSearch = {
    ...
    headers: {
      "X-RapidAPI-Key": "c388eb03f2msh4da41fa59501084p152988jsn30bf33681f23",
...
app.post("/liked", (req, res) => {
  ...
  if (currentUser && currentUserEmail) {
    db.collection("users").findOne({ Email: currentUserEmail }, (err, docs) => {
      ...
      if (
        likedArr.filter((song) => song.songID === songObj.songID).length > 0
      ) {
